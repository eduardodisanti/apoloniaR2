...
$query = "select Paciente, Trabajo, Fecha, descripcion, tiempo, Nombre from TrabSoc, Trabajos, Pacientes where Fecha >= '$FECHADESDE' and Fecha <='$FECHAHASTA' and Trabajo = id and Paciente = Cedula and (Entregado !=7 and Entregado != 5) $limit order by Paciente, Fecha,descripcion";
...
while($reg = fetch($q))
 {
	...
	$vence = date("Y-m-d", strtotime("+$tiempo day ".$fecha));
		...
		if($cmd=="bajar")
		  {
		   echo "<a href='venctrab.php?cmd=bajar&accion=bajar&pac=$cedula&trabajo=$trabajo&fechaTrab=$fecha&cmd=bajar&comando=seguir&Laboratorio=$Laboratorio&FECHADESDE=$FECHADESDE&FECHAHASTA=$FECHAHASTA'>Entregado</a>";
		  }
		echo "</td>";
		echo "<td>$vence</td>";
		echo "<td>$citado</td>"; 
