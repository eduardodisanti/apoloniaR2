...
function elegirProcedimiento(diente1, diente2, procedimiento, comentario) {
    ...
    texto = document.getElementById("tbComentario");

    texto.value = comentario;
    texto.selectionStart = 0;
    texto.selectionEnd   = 4096;
    ...
    ajax.onreadystatechange=function(){
    	if(ajax.readyState==4) {
    	   ...
    	   if(resultado=='ACK') {
			...
			// window.open("gastoepisodio.php?pac="+paciente+"&pieza="+diente1);
    	       }
    	   }
...
function eliminarParaHacer(diente1, diente2, procedimiento) {
    ...
    ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
    ajax.send("pac="+paciente+"&diente="+diente1+"&dienteHasta="+diente2+"&proc="+procedimiento+"&usr="+usuario);
}
...
function addRowToTable(diente1, diente2, procedimiento, nombreProc, texto){
  ...
  nodoLink.innerHTML = "<a href='javascript:elegirProcedimiento("+diente1+","+diente2+","+procedimiento+","+"\""+texto+"\""+")'>"+nombreProc+"</a>"; 
  cellLeft.appendChild(nodoLink);  cellLeft.appendChild(nodoLink);
...
function agregarHecho() {
   ...
   if(yaExisteProcedimiento(paciente, procedimiento, diente1, diente2) && procedimiento != 2 && procedimiento != 3) {
...
function ponerCuadroMensajes() {
  ...
  function colocarListaProcedimientos($esMedico) {
   ...
   if($esMedico) {
          ...
          echo "<OPTION value='2'>Apostilla administrativa</OPTION>\n";
    echo "</SELECT>";
...
//  echo "<form action='episodio3.php' method='submit'>";
  ...
  //colocarBocaTemporal($paciente);
  colocarEditor();
  colocarListaProcedimientos($medicoSES!=0);
  //colocarBotonAgregar();
