...
if(empty($f1) || empty($f2))
 {
   echo "<center><h2>Consultas por sucursal entre fechas</h2>";
   echo "<form action=\"conxsucproc.php\" method=\"post\">";
...
else
 {
   ...
   $q="select Sucursal, count(Horarios.Consultorio) as vistos, Procedimientos.Nombre as proc, count(Horarios.Procedimiento) from Horarios, Consultorios, Procedimientos where Consultorio=Consultorios.Codigo and Fecha >= '$f1' and Fecha <= '$f2' and Paciente != 0 and Procedimientos.Codigo = Horarios.Procedimiento group by Sucursal, proc order by Sucursal, proc";
