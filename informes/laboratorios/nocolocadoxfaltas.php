...
echo "<center><h4>Trabajos no colocados x faltas de pacientes</h4><center><hr>";
if(empty($comando))
 {
     echo "<form action='nocolocadoxfaltas.php' method=post>\n";
...
while($reg = fetch($q))
 {
    ...
    $cq = query("select * from Horarios where Paciente = $pac and Fecha >='$fecha' and Fecha <='$hoy' and Vino='N'");
    $rcq = fetch($cq);
    $cita = $rcq->Fecha;

    if(empty($cita)) // **** PUEDE HABER FALTADO CON AVISO ***
      {
          $cq = query("select * from Borrados where Paciente = $pac and Fecha >='$fecha' and Fecha <='$hoy'");
          $rcq = fetch($cq);
          $cita = $rcq->Fecha;
      }
