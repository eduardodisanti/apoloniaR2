...
if(empty($comando))
 {
     echo "<form action='../informes/venctrab1.php' method=post>\n";
...
while($reg = fetch($q))
 {
	...
	$vence = date("Y-m-d", strtotime("+$tiempo day ".$fecha));
		...
		if($cmd=="bajar")
		  {
		   echo "<a href='venctrab.php?cmd=bajar&accion=bajar&pac=$cedula&trabajo=$trabajo&fecha=$fecha&cmd=bajar&comando=seguir'>Entregado</a>";
		  }
		echo "</td>";
		echo "<td>$vence</td>";
		echo "<td>$citado</td>"; 
