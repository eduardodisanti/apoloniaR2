...
echo "<script>";
echo "
function modificar(numero, id, elemento, elemento1, rid, numero, remito, idvence) 
{
  var xid, precio, resultado;
    ...
    rfecha = document.getElementById(idvence);
    vence = rfecha.value;
...
//    alert(\"numero=\"+numero+\"&articulo=\"+id+\"&valor=\"+valor+\"&lote=\"+rid+\"&numero=\"+numero+\"&+\"&remito=\"+remito+\"&cantidad=\"+valor);
    ...
    ajax.send(\"numero=\"+numero+\"&articulo=\"+id+\"&valor=\"+valor+\"&lote=\"+valor2+\"&numero=\"+numero+\"&cantidad=\"+valor+\"&remito=\"+remito+\"&vence=\"+vence);
 }
}
";
echo "</script>";
...
echo "<tr bgcolor='#ccccff'><th>Orden de compra</th><th>Articulo</th><th>Remanente</th><th>Lote</th><th>Vence</th><th>Entregado</th></tr>";
...
$query="select articulo, articulos.vence as critico, nombre, fecha, cantidad, saldo, numero from OrdenDeCompra, articulos where $trozonumero and articulos.id = OrdenDeCompra.articulo and numero != 0 and cantidad !=0 order by nombre";
...
while($reg = fetch($qry))
 {
    ...
    echo "<td>";
    if($critico=='S')
       echo "   <input id='$rven' size=8 value='$vence'>";
    echo "</td>";
    echo "<td align=right>";
    if($remanente != 0)
       echo "<input id='$eid' size=9 value='$remanente' onBlur='modificar($numero, $id, \"$eid\", \"$eeid\", \"$rid\", $numero, $remito, \"$rven\")'>";
    else
       echo "&nbsp;";
    echo "</td></tr>";
 }
