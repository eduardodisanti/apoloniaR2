...
//	   $r = mysql_query("select * from MetaTrabTrab where ");

                $r = mysql_query("select ProcTrab.Procedimiento from ProcTrab, Episodios where ProcTrab.procedimiento = Episodios.Procedimiento and ProcTrab.Trabajo = $cod and Paciente = $pac");

//echo "select MetaTrabProc.Procedimiento from MetaTrabProc, MetaTrabTrab, Episodios where MetaTrabProc.Meta = MetaTrabTrab.Meta and MetaTrabProc.Meta = Episodios.Procedimiento and MetaTrabTrab.Trabajo = $cod and Paciente = $pac";

            $resp = mysql_num_rows($r);
       //    echo "Resp es $resp";
   ...
   mysql_close();
   if($asignados==0) {
      echo "<center><b>No hay ninguna indicacion en la historia que requiera trabajos de laboratorio</b></center>";
   }
}
