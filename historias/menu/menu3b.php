<?php
if(empty($coiusuario)) {

    session_start();
        $coiusuario= $_SESSION['coiusuario'];
    }

?>

<script type="text/javscript">
    /* preload images */
    var arrow1 = new Image(4, 7);
    arrow1.src = "menu/images/arrow1.gif";
    var arrow2 = new Image(4, 7);
    arrow2.src = "menu/images/arrow2.gif";
    </script>

    <div id="bar">
        <table cellspacing="0" cellpadding="0" id="menu1" class="XulMenu">
        <tr>
            <td>
                <a class="button" href="javascript:void(0)">Sistema</a>
                <div class="section">
                    <a class="item" href="index.php">Inicio</a>
                    <a class="item" href="../desconectar.php">Cerrar</a>
                </div>
            </td>
            <td>
                <a class="button" href="javascript:void(0)">Historias</a>
                <div class="section">
                     <a class="item" href="index.php?contenido=episodio.html">Ingresar un Episodio</a>
                     <a class="item" href="index.php?contenido=revision.html">Revision</a>
                </div>
            </td>
            <td>
                <a class="button" href="javascript:void(0)">Pacientes</a>
                <div class="section">
                         <a class="item" href="index.php?contenido=../utils/padron.php">Consultar el padron</a>
                         <a class="item" href="index.php?contenido=../agenda/autorizar.php">Autorizaciones</a>
                </div>
            </td>
            <td>
                <a class="button" href="javascript:void(0)">Listas</a>
                <div class="section">
                         <a class="item" href="index.php?contenido=../agenda/listaconf.php">Lista de consulta</a>
                         <a class="item" href="index.php?contenido=../agenda/listapac.php">Lista de un paciente</a>
                </div>
            </td>
            <td>
               <a class="button" href="javascript:void(0)">Laboratorio</a>
               <div class="section">
                         <a class="item" href="index.php?contenido=../informes/pendalab.php?cmd=bajar">Pendientes de Laboratorio</a>
			 <a class="item" href="index.php?contenido=../informes/venctrabxmed.php">Trabajos de laboratorio por consulta</a>
               </div>
            </td>
            <td>
                <a class="button" href="javascript:void(0)">Ayuda</a>
                <div class="section">
                    <a class="item" href="manuales/calidad.php">Manual de Calidad</a>
                    <a class="item" href="manuales/sistema.php">Manual del sistema</a>
		    <a class="item" href="http://hermes.kcreativa.com/script/solicitud.php?cliente=111&quien=C&Husuario=<?php echo $coiusuario; ?>" target="_blank">Pedir soporte tecnico</a>
		    <a class="item" href="index.php?contenido=../bugdb/reportar.php">Reportar errores</a>
                </div>
            </td>
            <td width=200px>
                &nbsp;
            </td>    
            <td>
                <a class="button" href="javascript:history.go(-1)"><img src="../img/back.png" border=0 align=middle> Atras</a>
            </td>
            <td>
                <a class="button" href="index.php"><img src="../img/gohome.png" border=0 align=middle>Inicio</a>
            </td>	    
            <td>
                <a class="button" href="javascript:contenido.location.reload()"><img src="../img/reload.png" border=0 align=middle>Recargar</a>
            </td>	    

            <td>
                <a class="button" href="javascript:contenido.window.print()"><img src="../img/fileprint.png" border=0 align=middle>Imprimir</a>
            </td>
            <td>
                <a class="button" href="../logoncentral.php?situacion=yalogeado"><img src="../img/exit.png" border=0 align=middle>Salir</a>
            </td>	            
	</tr>
        </table>
    </div>

    <script type="text/javascript">
    var menu1 = new XulMenu("menu1");
    menu1.arrow1 = "images/arrow1.gif";
    menu1.arrow2 = "images/arrow2.gif";
    menu1.init();
    </script>
