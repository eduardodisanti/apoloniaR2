...
echo "<center><h4>Vencimiento de trabajos de laboratorio (<b>PT TPE 01</b>)</h4></center><hr>";
if(empty($comando))
 {
...
else
 {
    ...
    $hoy = Date("Y-m-d");
     ...
     if($accion=="bajar")
        {
	  $query = "update TrabSoc set Entregado=5 where Paciente=$pac and Trabajo=$trabajo and Fecha='$fechaTrab'";

	  $q = query($query);
	  $query = "insert into HistTrabSoc values($pac, $trabajo, '$hoy', $Entregado, $Laboratorio)";
          $q = query($query);
	}
...
$query = "select Paciente, Trabajo, Fecha, Trabajos.descripcion, tiempo, Nombre, EstadosTrabajo.descripcion as descestado from TrabSoc, Trabajos, Pacientes, EstadosTrabajo where Fecha >= '$FECHADESDE' and Fecha <='$FECHAHASTA' and Trabajo = id and Paciente = Cedula and (Entregado !=7 and Entregado != 5) and EstadosTrabajo.Codigo = Entregado $limit order by Paciente, Fecha";
...
while($reg = fetch($q))
 {
	...
	$vence = date("Y-m-d", strtotime("+$tiempo day ".$fecha));
		...
		if($cmd=="bajar")
		  {
		   echo "<a href='venctrab.php?cmd=bajar&accion=bajar&pac=$cedula&trabajo=$trabajo&fechaTrab=$fecha&cmd=bajar&comando=seguir&Laboratorio=$Laboratorio&FECHADESDE=$FECHADESDE&FECHAHASTA=$FECHAHASTA'>$estado</a>";
		  }
		echo "</td>";
		echo "<td>$vence</td>";
		echo "<td>$citado</td>"; 
