...
function facturaPaga($serie, $numero, $laboratorio) {
    ...
    $query = "select * from RecFact where LabRecibo=$laboratorio and SerieFact='$serie' and NumeroFact='$numero'";
...
if($nivel < 7) {
     ...
     die("<h3>No esta autorizado para ejecutar este comando</h3>");
}
...
if(empty($comando))
 {
     ...
     echo "</tr>";
     echo "<tr>";
     echo "   <td>Emitir preventivo                   )</td>";
     echo "   <td><input type='checkbox' name='PREVENTIVO'></td>";
     echo "</tr>";
...
echo "Lista de trabajos realizados entre $FECHADESDE y $FECHAHASTA $xprev<br>";
...
$query = "select FactLab.Laboratorio as numlab, Laboratorios.descripcion as nomlab ,Serie,Numero,Fecha, Trabajo, Trabajos.descripcion desctra, Pacientes.Nombre nompac, Cedula, Pieza, histprec.precio as Costo, histprec.impuestos as iva from FactLab, Trabajos, Pacientes, Laboratorios, histprec where Fecha > '$FECHADESDE' and Fecha <='$FECHAHASTA' and Trabajo = Trabajos.id and Paciente = Cedula $limit and FactLab.Laboratorio = Laboratorios.id and 
