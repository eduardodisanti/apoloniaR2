...
function sacarListaRecibos($serie, $numero, $labo) {
     ...
     while($reg = fetch($q))
	   ...
	   echo "<tr bgcolor='#cccccc'><td>&nbsp;&nbsp;</td><td colspan=2>($ser)-$num</td><td align=right>$ximporte</td></tr>";
...
if(empty($comando))
 {
     ...
     echo "<form action='cuentadelab.php' method=post>\n";
...
while($reg = fetch($q))
 {
	...
	$labo      = $reg->lab;
        ...
        if($nombreLab!=$labAnt)
	  {
        ...
        echo "<tr bgcolor='#ffffff'><td>$serie</td><td>$numero</td><td>$fecha</td><td align='right'>$ximporte</td></tr>";
	$guita+=$importe;
