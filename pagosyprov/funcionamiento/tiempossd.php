...
if(empty($comando))
 {
     echo "<form action='tiempossd.php' method=post>\n";
...
// ** Para cada paciente
     $q1 = mysql_query("select Paciente from Horarios where Paciente !=0 and Fecha >= '$FECHADESDE'  and Fecha <= '$FECHAHASTA' group by Paciente");
     while($reg=mysql_fetch_object($q1))
	   ...
	   if(empty($fechaInicial))
	    {
           	$q2 = mysql_query("select Fecha from Horarios where 	  Paciente=$pac  and Fecha >= '$FECHADESDE' and Fecha <= '$FECHAHASTA' order by Fecha  desc");
           	$regq2 = mysql_fetch_object($q2);
           	$fechaInicial = $regq2->Fecha;
		$primera=0;
		$norev=1;
           ...
           $q3 = mysql_query("select Fecha from Horarios where Paciente=$pac and Fecha > '$fechaInicial' and Fecha <= '$FECHAHASTA' order by Fecha");
