...
<form name="anotar" action="anotarauto.php" method="post">
<?
 ...
 if(empty($comando))
  {
   ...
   echo "<input type=\"hidden\" name=\"tipocmd\" value=\"$tipocmd\">\n";
  }
...
/*    $coment=mysql_query("select * from Comentarios where Paciente=$ci");
    ...
    echo "</table>";
*/
    echo "<b>Paso 2</b>, ($tipocmd)Elegir un procedimiento <a href=\"../historias/revision.php?paciente=$ci\" target=\"_revision\">Revision</a> <a href='anotarauto.php?ci=$ci&comando=Paso2&tipocmd=$tipocmd'>Recargar<img src='../img/reload.png' border=0 align=middle></a><br>Se atiende con $medico_actual->nombre"; 
    ...
    $query=mysql_query("select Nombre, Codigo, Pieza, Especialidad from Procedimientos, ParaHacer where Paciente=$ci and Procedimiento = Codigo order by Nombre,Pieza") or 
...
echo "&nbsp;&nbsp;<input type=\"checkbox\" name=\"vertodo\" value=\"$vertodo\">Ver otras sucursales <input type=\"submit\" name=\"boton\" value=\"Ver\">\n";
    ...
    echo "   <td>";
    echo "      <a href=\"anotarauto.php?comando=Paso3&fechahasta=$fechahasta&fechadesde=$fechadesde&ci=$ci&orden=1&xpac=$xpac&boton=nada&proc=$proc&tipocmd=$tipocmd\">Fecha</a>";
    echo "   </td>\n";
    echo "   <td>Hora</td>\n";
    echo "   <td>Consultorio</td>\n";
    echo "   <td>Turno</td>\n";
    echo "   <td>";
    echo "      <a href=\"anotarauto.php?comando=Paso3&fechahasta=$fechahasta&fechadesde=$fechadesde&ci=$ci&orden=2&xpac=$xpac&boton=nada&proc=$proc&pieza=$pieza&tipocmd=$tipocmd\">Medico</a>";
    echo "</td>\n";
    echo "   <td>";
    echo "      <a href=\"anotarauto.php?comando=Paso3&fechahasta=$fechahasta&fechadesde=$fechadesde&ci=$ci&orden=3&xpac=$xpac&boton=nada&proc=$proc&pieza=$pieza&tipocmd=$tipocmd\">Especialidad</a>";
    echo "</td>\n";
	    ...
	    $color="#ffa500";
         echo "<tr bgcolor=$color>\n";
          if($cantidad >= $limite || $regsup->Suplente == 98)
                ...
                echo "   <td><a href=\"mostrarconsulta.php?Fecha=$Fecha&Turno=$Turno&Consultorio=$Consultorio&Turno=$Turno&Medico=$codmed&Proc=$codproc&Cedula=$Xci&cmd=anotar&pieza=$pieza&Xpac=$xpac&tipocmd=$tipocmd\">$xdiasemana<font size=+0><b>$Fecha</b></font></a></td>\n";
