  ...
  if($coisucursal_ses=="CENTRAL")
    {
     ...
     if(strlen($hhAhora) < 2)
      ...
      $query = "select medico from TablaUrgencias, Medicos where sucursal = '$coisucursal' and Medicos.Numero = medico and Especialidad != 9 and Especialidad != 2 and Especialidad != 10 order by tiempo";

      $qry = mysql_query($query);

      $encontre = false;
      while(!$encontre && $rreg = mysql_fetch_object($qry)) {

              $medicoPool = $rreg->medico;

              $qquery = "select Medico, Consultorio, Sucursal, count(*) as cuenta from Horarios, Consultorios where Fecha='$hoy' and Medico=$medicoPool and Turno>=$turno and Turno < $turnoLimite and Consultorios.Codigo = Consultorio and Consultorios.Sucursal = '$coisucursal' group by Medico";

 $error = mysql_error();
 /* if(!empty($error))
    die($qquery." - ".mysql_error());
*/
	    ...
	    $turnoLimite = $turno + 9;
            ...
            $qquery = "select Medico, Consultorio, Sucursal from Horarios, Consultorios where Fecha='$hoy' and Turno>=$turno and Turno < $turnoLimite and Consultorios.Codigo = Consultorio and Consultorios.Sucursal = '$coisucursal' group by Medico limit 1";

	    $qry = mysql_query($qquery);
	    $reg = mysql_fetch_object($qry);
	    $maxCons = $reg->Consultorio;
	    $med = $reg->Medico;
     }
     if(!$encontre)
        die("No se encontro medico, error");
 ...
 //     die($query);
     ...
     $timestamp = time();
     // echo $query;
      $query = "insert into TablaUrgencias values($med,'$coisucursal','$hoy', $timestamp)";
      mysql_query($query);
      $query = "update TablaUrgencias set ultima='$hoy', tiempo=$timestamp where medico=$med and sucursal='$coisucursal'";
      mysql_query($query);
