...
if($comando=="Paso2")
 ...
 abonar</b> </font></td>";
   ...
   $trancar="NO";
    ...
    $suspendido = "N";
    if($suspendido=="S")
       die("Paciente suspendido, no puede confirmar");
    echo "<hr>\n";
...
// *** Consulta de ordenes pagadas
         $fechaAAMM = substr($fecha,0,7);
         $queryRepositorio = mysql_query("select * from tablord where paciente=$ci and fecha='$fechaAAMM' and TipoOrden='$tipord'");

         $regRepositorio = mysql_fetch_object($queryRepositorio);
         $ordenesPagadas = $regRepositorio->haber - $regRepositorio->debe;
         if(empty($ordenesPagadas) || $ordenesPagadas < 0)
                $ordenesPagadas=0;
		
// *** Consulta de ordenes de taller pagadas
         $fechaAAMM = substr($fecha,0,7);
         $queryRepositorio = mysql_query("select * from tablord where paciente=$ci and fecha='$fechaAAMM' and TipoOrden='H'");
         $regRepositorio = mysql_fetch_object($queryRepositorio);
	 $tallerPagado = $regRepositorio->haber - $regRepositorio->debe;
	 if(empty($tallerPagado) || $tallerPagado < 0)
	                  $tallerPagado=0;

         $deudaOrtFija = deudaOrtFija($ci);
	 if($especialidad == 6 && $deudaOrtFija > 2)
          {
            echo "<script> alert('Segun nuestros registros el paciente debe $deudaOrtFija cuotas de ortodoncia fija no puede confirmarse');</script>";

	    die("<hr><h3>Pasar por administracion</h3>");
          }
	...
	echo "   <td>$medico</td><td>$nommed $xsup</td><td>$consultorio</td><td>$turno</td><td>$nproc</td><td>$numero</td>\n";
	echo "</tr>";
	...
	if($especialidad==6) //==6 
	  {
		...
		echo "  <input type='text' name='cantord' value='$ordenesPagadas' size=2>\n";
		echo "   Ordenes de taller";
		echo "  <input type='text' name='taller'  value='$tallerPagado' size=3>\n";
		echo " </td>";
	   } else
         ...
         if($suspendido=="N")
          {
           ...
           if(!empty($error))
              {
                  echo "<br>No se pudo eliminar de la tabla de faltas : <b>$error</b><br>";
              } else
