   ...
   if($coisucursal_ses=="CENTRAL")
    {
     ...
     if(strlen($hhAhora) < 2)
      ...
      $query = "select * from Medicos and Especialidad != 9 and Especialidad != 4";
      $requery = mysql_query($query);

      $query = "select medico from TablaUrgencias where sucursal = '$coisucursal' order by ultima";
      ...
      $max = 99 + $max + 1;
      $query="insert into Horarios values('$hoy', '$maxCons',$turno,'$ahora', $maxMed, $Cedula, 'N','S',693,'S','$ahora',$usr->funcionario, $max)";
